...
switch($_POST["tpl"]){
   case "get_usrs":
	...
	case "get_cate_list":#获取文章栏目信息
        ...
        $cate_info=$pd->query("select * from `main_art_category` where topic_id={$topic_id} order by odx")->fetchAll(PDO::FETCH_ASSOC);
	    $root=array('id'=>'0','name'=>'根目录','tcid'=>'0','taid'=>'0','topic_id'=>$topic_id,'iconOpen'=>"/widget/zTree/css/zTreeStyle/img/diy/1_open.png",'iconClose'=>"/widget/zTree/css/zTreeStyle/img/diy/1_close.png");
	    array_unshift($cate_info,$root);
	    echo json_encode($cate_info);
	    break;
	case "getAll_group_art_auth":
	    $rs=$pd->query("select group_id,art_publish_auth,art_audit_auth,cate_audit_auth from `osa_user_group`");
		echo json_encode($rs->fetchAll(PDO::FETCH_ASSOC));
	    break;
	case "get_artcate_new":
	    $topic_id=isset($_POST['topic_id'])?$_POST['topic_id']:0;
		$result = $pd->query("select id,name,pid from main_art_category where topic_id={$topic_id} order by odx asc")->fetchAll(PDO::FETCH_ASSOC); 
		$tree_arr=get_tree_arr($result,0);
		$tree_html=get_tree_html($tree_arr,$html="");
		echo $tree_html;
		break;
	case "get_artcate_push":
	    $push_cate=isset($_POST['push_cate'])?$_POST['push_cate']:'';
		if($push_cate!=''){
			$result = $pd->query("select A.id,A.name,B.name as tname from main_art_category A left join topic B on A.topic_id=B.id where A.id in ({$push_cate}) order by A.topic_id asc")->fetchAll(PDO::FETCH_ASSOC); 
		    echo json_encode($result);
		}
		break;
	case "get_artcate_mynew":#发文权限的栏目
		...
		if(!empty($uid)){
		    ...
		    $result = $pd->query("select id,name,pid from main_art_category where topic_id={$topic_id} and id in (".$art_publish_auth.") order by odx asc")->fetchAll(PDO::FETCH_ASSOC); 
			$tree_arr=get_tree_arr($result,0);
			$tree_html=get_tree_html($tree_arr,$html="");
			echo $tree_html;
		}
		break;
	case "get_artcate_mynew_sh":#审核权限的栏目
		//$uid=isset($_POST['uid'])?$_POST['uid']:"";
		$topic_id=isset($_POST['topic_id'])?$_POST['topic_id']:0;
		if(!empty($uid)){
			$cate_audit_auth=$pd->query("select oug.cate_audit_auth from act_member am LEFT JOIN osa_user_group oug on am.user_group=oug.group_id where am.id='".$uid."'")->fetchColumn(0);
		    $result = $pd->query("select id,name,pid from main_art_category where topic_id={$topic_id} and id in (".$cate_audit_auth.") order by odx asc")->fetchAll(PDO::FETCH_ASSOC); 
			$tree_arr=get_tree_arr($result,0);
			$tree_html=get_tree_html($tree_arr,$html="");
			echo $tree_html;
		}
